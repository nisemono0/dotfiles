...
mountusb() {
	chosen="$(echo "$usb_drives" | dmenu $DMENU_OPTIONS -fn "$DMENU_FN" -p "Mount which drive" | awk '{print $1}')"
	[ -z "$chosen" ] && exit 1
	notify-send "$(udisksctl mount -b "$chosen")" -i drive-harddisk || notify-send -u critical "There was a problem while mounting"
}

mountandroid() {
	chosen="$(echo "$android_drives" | dmenu $DMENU_OPTIONS -fn "$DMENU_FN" -p "Which Android device" | cut -d ':' -f 1)"
	[ -z "$chosen" ] && exit 1

	mount_point="$(find "$HOME" -maxdepth 3 -not -path "*.*" -type d 2>/dev/null | 
		dmenu $DMENU_OPTIONS -fn "$DMENU_FN" -p "Type in mount point")"
	mount_point="${mount_point/#\~/$HOME}"
	[ -z "$mount_point" ] && exit 1

	if [ ! -d "$mount_point" ]; then
		askmkdir="$(printf "No\\nYes" | dmenu $DMENU_OPTIONS -fn "$DMENU_FN" -p "Create: $mount_point")"
...
askmount() {
	case "$(printf "USB\\nAndroid" | dmenu $DMENU_OPTIONS -fn "$DMENU_FN" -p "Mount a USB drive or Android device")" in
		"USB")
			mountusb
			;;
		"Android")
