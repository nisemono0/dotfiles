...
declare -r -x PREVIEW_ID="fzf-preview"
...
function ADD_PLACEMENT {
    ...
    _add_command[y]=$(("${adjusted_placement_rect[y]}" + 1))
    _add_command[width]="${adjusted_placement_rect[width]}"
    _add_command[height]=$(("${adjusted_placement_rect[height]}" - 1))
