...
playsong() {
    songpos=$(mpc playlist -f "%position%.[%artist%|%albumartist%|%file%] - %title%" | \
        dmenu "${DMENU_ARGS[@]}" -fn "Noto Sans CJK JP:style=Bold:size=9" -l 15 -p "Play" | \
        cut -f 1 -d '.')
    [ -z "$songpos" ] && exit
...
addsongplay() {
    songfile=$(mpc listall | dmenu "${DMENU_ARGS[@]}" -fn "Noto Sans CJK JP:style=Bold:size=9" -l 15 -p "Add & Play")
    [ -z "$songfile" ] && exit
