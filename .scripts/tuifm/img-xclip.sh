...
if [[ "$filetype" == image/* ]]; then
    if [[ "$filetype" == image/png ]]; then
        ...
        magick "$filepath" png:- | xclip -selection clipboard -target image/png -i &> /dev/null && notify-send "Image copied to clipboard" -i "$filepath" || notify-send -u critical "Something went wrong"
    fi
else
