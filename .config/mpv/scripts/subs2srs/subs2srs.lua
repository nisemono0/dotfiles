...
local config = {
    ...
    animated_snapshot_height = 250, -- positive integer or -2 to scale it maintaining ratio (width must not be -2 in that case)
    animated_snapshot_quality = 10, -- positive integer between 0 and 100 (100 included)

    -- Audio clips
    audio_format = "opus", -- opus or mp3
    audio_bitrate = "24k", -- from 16k to 32k
    ...
    miscinfo_field = "From", -- misc notes and source information field
    ...
    vocab_field = "Expression", -- target word field should be equal to {expression} field in Yomichan
    vocab_audio_field = "Audio", -- target word audio should be equal to {audio} field in Yomichan
}
...
menu.keybindings = {
    ...
    { key = 'Ctrl+g', fn = menu:with_update { encoder.snapshot.toggle_animation } },
    { key = 'h', fn = _ { play_control.sub_seek, 'backward' } },
    { key = 'l', fn = _ { play_control.sub_seek, 'forward' } },
    { key = 'H', fn = _ { play_control.sub_seek, 'backward', true } },
    { key = 'L', fn = _ { play_control.sub_seek, 'forward', true } },
    { key = 'Ctrl+h', fn = _ { play_control.sub_rewind } },
    { key = 'Ctrl+H', fn = _ { play_control.play_till_sub_end } },
    { key = 'Ctrl+L', fn = _ { play_control.play_till_next_sub_end } },
...
function menu:print_bindings(osd)
    if self.hints_state.get() == 'global' then
        ...
        osd:tab():item('a: '):text("Toggle this menu"):newline()
        osd:tab():item('Ctrl+c: '):text('Copy primary subtitle to clipboard'):newline()
        osd:tab():item('Ctrl+Alt+c: '):text("Copy secondary subtitle to clipboard"):newline()
        osd:italics("Press "):item('i'):italics(" to show mpvacious."):newline()
